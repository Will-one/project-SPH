  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      >
        ...
        <p @click="changeDefault(address, addressList)">
          ...
          <span class="s3" v-show="address.isDefault">默认地址</span>
        </p>
      </div>
      <div class="line"></div>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ currDefaultAddress.userAddress }}</span>
        收货人：<span>{{ currDefaultAddress.consignee }}</span>
        <span>{{ currDefaultAddress.phoneNum }}</span>
      </div>
    </div>
...
import { reqreqSubmitOrder } from "@/api"
  export default {
    name: 'Trade',
    data() {
      return {
    ...
    methods: {
      ...
      async submitOrder(){
          ...
          let params = {
            consignee:this.currDefaultAddress.consignee,
            consigneeTel:this.currDefaultAddress.phoneNum,
            deliveryAddress:this.currDefaultAddress.userAddress,
            paymentWay:'ONLINE',
          ...
          // 发送申请
          let res = await reqreqSubmitOrder(tradeNo,params)
          if (res.code == 200){
            orderId = res.data
